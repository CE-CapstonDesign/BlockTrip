    ...
    private String createRestaurantPrompt(TravelRequest request) {
        ...
        return String.format(
                "목적지: %s, 출발일: %s, 도착일: %s\n" +
                        ...
                        "목록 형식으로 최대 30개의 실제 존재하는 음식점를 제시해주세요. 각 음식점의 이름만 제공하고, 이를 예시와 같이 쉼표 only Comma(, )로만 구분해 나열해주세요. (예시: 광주형무소역사관, 아시아문화전당, 전남대학교, 광주황톳길)",
    ...
    private String createPlacePrompt(TravelRequest request) {
        ...
        return String.format(
                "목적지: %s, 출발일: %s, 도착일: %s\n" +
                        ...
                        "목록 형식으로 최대 30개의 관광지를 제시해주세요. 각 관광지의 이름만 제공하고, 이를 예시와 같이 쉼표 Comma(, )로만 구분해 나열해주세요. (예시: 일식당, 데미안, 일본삼시세끼, 일알리스)",
                common.getDestinationLocation(),
                common.getDepartureDate(),
                common.getArrivalDate(),
                common.getDestinationLocation(),
