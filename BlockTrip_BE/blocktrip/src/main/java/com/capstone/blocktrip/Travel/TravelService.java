    ...
    private String createRestaurantPrompt(TravelRequest request) {
        ...
        return String.format(
                "목적지: %s, 출발일: %s, 도착일: %s\n" +
                        ...
                        "목록 형식으로 최대 20개의 실제 존재하는 음식점를 제시해주세요. 각 음식점의 이름만 제공하고, 개행(newLine)을 never 사용하지 말고 이를 쉼표 only Comma(, )로만 구분해 나열해주세요.\",",
    ...
    private String createPlacePrompt(TravelRequest request) {
        ...
        return String.format(
                "목적지: %s, 출발일: %s, 도착일: %s\n" +
                        ...
                        "목록 형식으로 최대 20개의 관광지를 제시해주세요. 각 관광지의 이름만 제공하고, 개행(newLine)을 절대로 never 사용하지 말고 이를 쉼표 Comma(, )로만 구분해 나열해주세요.",
                common.getDestinationLocation(),
                common.getDepartureDate(),
                common.getArrivalDate(),
                common.getDestinationLocation(),
